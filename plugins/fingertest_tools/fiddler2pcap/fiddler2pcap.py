...
if os.path.isdir(options.fiddler_raw_dir):
    ...
    for xml_file in m_file_list:
        ...
        print(options.fiddler_raw_dir + fid + "_s.txt")

        temp_content = str(req).split("HTTP/1.1")[0]
        #print(temp_content)
        find2 = temp_content.find(" https://")
        #print(find2)
        index = temp_content.index("/")
        index = temp_content.index("/", index + 1)
        index = temp_content.index("/", index + 1)
        httpf = temp_content[find2:index]
        print("replace: " + httpf)
        req = req.replace(httpf, " ", 1)

        resp = open(options.fiddler_raw_dir + fid + "_s.txt",'rb')
        resp = resp.read()
